...
class V040MemberNeedPermissionsTestTMP extends AcceptanceTestCase
{
    ...
    {
        $response = $this->apiMemberNeedsPost(self::$client);
        $this->assertEquals(403, $response->getStatusCode());

        $response = $this->apiMemberNeedsGetCollection(self::$client);
        $this->assertEquals(403, $response->getStatusCode());

        $response = $this->apiMemberNeedsGet(self::$client, 'fake');
        $this->assertEquals(403, $response->getStatusCode());

        $response = $this->apiMemberNeedsDelete(self::$client, 'fake');
        $this->assertEquals(403, $response->getStatusCode());
    }
    ...
    private function assertCreatePermissionForOneRole(string $requesterRole): void
    {
        foreach (RoleEnum::toArray() as $subjectRole) {
            ...
            $fakeData = $this->apiMemberNeedsFakePostData($subjectRole);
            $response = $this->apiMemberNeedsPost(self::$client, $fakeData);
            $this->assertEquals($isPermitted ? 201 : 403, $response->getStatusCode(), $requesterRole . ' - ' . $subjectRole);
            if ($isPermitted) {
    ...
    private function assertReadPermissionForOneRole(string $requesterRole): void
    {
        ...
        $response = $this->apiMemberNeedsGet(self::$client, $firstId);
    ...
    {
        $isPermitted = self::$configurationService->hasPermissionBoolean(
            PermissionEnum::PARKING_MEMBER_NEED_UPDATE(),
            new RoleEnum($requesterRole)
        );
        ...
        $response = $this->apiMemberNeedsPut(self::$client, $itemId, $requestData);
        $this->assertEquals($isPermitted ? 200 : 403, $response->getStatusCode(), $requesterRole);
        if ($isPermitted) {
            $this->assertArrayHasKey('id', $this->jsonDecode($response->getContent()));
            $itemData = $this->jsonDecode($response->getContent());
