...
class V010AvailabilityPermissionsTest extends AcceptanceTestCase
{
    ...
    private function assertAnonymousActions(): void
    {
        ...
        $response = $this->apiAvailabilitiesGet(self::$client, 'fake');
        $this->assertEquals(403, $response->getStatusCode());

        $response = $this->apiAvailabilitiesDelete(self::$client, 'fake');
        $this->assertEquals(403, $response->getStatusCode());
    }
    ...
    private function assertCreatePermissionForOneRole(string $requesterRole): void
    {
        $isPermitted = self::$configurationService->hasPermissionBoolean(
            PermissionEnum::PARKING_AVAILABILITY_CREATE(),
            new RoleEnum($requesterRole)
        );

        $fakeData = $this->apiAvailabilitiesFakePostData($this->calculateFakeYear());
        $response = $this->apiAvailabilitiesPost(self::$client, $fakeData);
        $this->assertEquals($isPermitted ? 201 : 403, $response->getStatusCode(), $requesterRole);
        if ($isPermitted) {
    ...
    {
        $isPermitted = self::$configurationService->hasPermissionBoolean(
            PermissionEnum::PARKING_AVAILABILITY_READ(),
            new RoleEnum($requesterRole)
        );
        $response = $this->apiAvailabilitiesGetCollection(self::$client);
    ...
    {
        $isPermitted = self::$configurationService->hasPermissionBoolean(
            PermissionEnum::PARKING_AVAILABILITY_UPDATE(),
            new RoleEnum($requesterRole)
        );
